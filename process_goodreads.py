...
df['Series'] = df['Title'].str.extract(r'\(([^,]+), #\d+\)', expand=False)  # Extract series
...
total_books = len(books_read)  # Only read books
total_pages = books_read['Number of Pages'].sum()
total_words = books_read['Estimated Word Count'].sum()
total_read = len(books_read)  # Same as total_books in this context
...
book_list['Series'] = book_list['Series'].apply(lambda x: x if pd.notna(x) else None)  # Convert NaN to None
...
stats = {
    ...
    'series_counts': {k: int(v) for k, v in series_counts.items()},  # Ensure integers
    'book_list': book_list,
